...
parse_page <- function (url, visited_links) {
  ...
  if (grepl(?????, current_url, perl = TRUE)) {
  ...
  # NOTE: While you can extract all links in a single XPath expression, the one
  # needed here is very long and may be too complex. So if you're struggling,
  # you can also try to split that up into multiple expressions so that it's
  # easier to retrieve all those links.
  working_links <- xpathSApply(tree, ?????)

  # Now that we have a list of (potentially) interesting links, we have to
  ...
  for (link in working_links) {
    ...
    # Ensure that the link links to some "/wiki/something" page.
    # (NOTE: Wikipedia uses relative links)
    if (!grepl(?????, href, perl = TRUE)) {
      next
    }
    ...
    # Check if the link would link to a Disambiguation page (that normally ends
    # with "(disambiguation)")
    if (grepl(?????, href, perl = TRUE)) {
      next
    }

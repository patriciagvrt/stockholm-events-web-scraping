...
# Write it to a file "wikipedia.html"
writeLines(html, "wikipedia.html")
# At a later point, you can read it in:
lines <- readLines('wikipedia.html')
# Turn the lines into a single string
...
#Another Example: Etienne's site
home <- 'https://ollion.cnrs.fr/dsss25/'
url <- 'https://ollion.cnrs.fr/wp-login.php?action=postpass'
post_password <- 'DSSS25'
Submit <- 'Valider'

parameters <- list(
  redirect_to = 'https://ollion.cnrs.fr/dsss25/',
  post_password = post_password
)

response <- POST(url, body = parameters, encode = 'form')
etienne_cookies <- cookies(response)

# Now we can request the home page
response2 <- GET(home, set_cookies(.cookies = as.character(etienne_cookies)))

content(response2, as="text")
writeLines(content(response2, as="text"), "dsss.html")
...
response$headers # not mentioning encoding
stringi::stri_enc_detect(response$content) # very likely ISO-8859-1, try it
